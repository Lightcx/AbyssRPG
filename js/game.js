...
(function (root) {
  ...
  Game.prototype.restoreFloor = function (cache) {
    ...
    this.monsters = (cache.monsters || []).map((saved) => {
      ...
      // 抗性 / 护甲是按层数 · 难度算出来的，老缓存里没有（或用的旧曲线）→ 这里重算
      const ml = mo.mlvl || this.mlvl;
      mo.res = G.monsterResists(ml, this.diffIdx, this.floor);
      if (mo.def) mo.armor = G.monsterArmor(mo.def.armor, ml, this.diffIdx, this.floor);
      return mo;
    }).filter(Boolean);
